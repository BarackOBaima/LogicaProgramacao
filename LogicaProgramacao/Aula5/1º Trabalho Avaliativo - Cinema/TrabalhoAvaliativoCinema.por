...
/* O exercício pede que façamos um sistema de reserva de lugares
 * em um cinema, pedindo que digamos a letra e o número de cada
 * fileira. Pede também que diga quando a poltrona estiver ocupada,
 * e que liste e evidencie 
 * 
 * 
 * 
 */
...
//Constantes
    const inteiro FILEIRAS = 16
//Fileiras de A a P
    const inteiro POLTRONAS_POR_FILEIRA = 15
//15 poltronas por fileira

//Variáveis globais
    logico poltronas[FILEIRAS][POLTRONAS_POR_FILEIRA]
//Matriz de poltronas (true = ocupada, false = disponível)
    cadeia letras_fileiras[FILEIRAS] = {"A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L", "M", "N", "O", "P"}
//Letras das fileiras
    ...
    funcao inicializarPoltronas()
    {
        para (inteiro i = 0; i < FILEIRAS; i++) {
            para (inteiro j = 0; j < POLTRONAS_POR_FILEIRA; j++) {
                poltronas[i][j] = falso
//Todas as poltronas começam disponíveis
            }
        }
    }
//Função para exibir o mapa de poltronas (com fileiras e poltronas invertidas)
    funcao exibirMapaPoltronas()
    {
        escreva("\nMapa de Poltronas:\n")
        escreva("     ")
//Espaço para alinhar as fileiras
        para (inteiro i = 0; i < FILEIRAS; i++) {
            escreva(letras_fileiras[i], " ")
//Exibe as letras das fileiras na horizontal
        }
        escreva("\n")

//Exibe as poltronas na vertical e os estados na horizontal
        para (inteiro j = 0; j < POLTRONAS_POR_FILEIRA; j++) {
            se(j + 1 < 10){
            	escreva(j + 1, "    ")
//Exibe o número da poltrona na vertical
            }senao {
            	escreva(j + 1, "   ")
            }
            para (inteiro i = 0; i < FILEIRAS; i++) {
    ...
    funcao logico fileiraCheia(inteiro fileira)
    {
        para (inteiro j = 0; j < POLTRONAS_POR_FILEIRA; j++) {
            se (nao poltronas[fileira][j]) {
                retorne falso
//A fileira não está cheia
            }
        }
        retorne verdadeiro
//A fileira está cheia
    }
...
//Compara a letra com cada elemento do vetor de letras
        ...
        retorne -1 
//Retorna -1 quando letra não encontrada
    }
...
//Solicita a poltrona
                    escreva("Digite o número da poltrona (1 a 15): ")
                    inteiro poltrona
                    leia(poltrona)
